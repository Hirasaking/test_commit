...
class CreateSalesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('sales', function (Blueprint $table) {
            $table->unsignedBigInteger('sale_id')->comment('販売番号')->autoIncrement();
            $table->unsignedTinyInteger('market')->comment('販売先');
            $table->string('product_name', 128)->comment('商品名');
            $table->unsignedSmallInteger('sales_volume')->comment('数量');
            ...
            $table->unsignedInteger('sales_amount')->comment('合計金額');
